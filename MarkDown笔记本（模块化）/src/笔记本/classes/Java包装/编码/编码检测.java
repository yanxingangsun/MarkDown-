 ...
 */ 
public class 编码检测{
    public static 编码枚举 检测文件编码(String filePath) {
        编码检测 s = new 编码检测();  
        return 编码枚举.values()[s.检测编码(new File(filePath))];
    }
    ...
    public static void main(String argc[]) {
        编码检测 sinodetector;
        int result = 编码枚举.OTHER.ordinal();
        int i;
        sinodetector = new 编码检测();
        for (i = 0; i < argc.length; i++) {
            if (argc[i].startsWith("http://") == true) {
                try {
                    result = sinodetector.检测编码(new URL(argc[i]));
                } catch (Exception e) {
                ...
                result = sinodetector.检测编码(new File(argc[i]));
            }
            System.out.println(编码枚举.values()[result].友好名称);
        }
    }

    /**
     * Function : detectEncoding Aruguments: URL Returns : One of the encodings from the Encoding enumeration (GB2312, HZ, BIG5, EUC_TW, ASCII, or OTHER) Description: This function looks at the URL
     * contents and assigns it a probability score for each encoding type. The encoding type with the highest probability is returned.
     */
    public int 检测编码(URL testurl) {
        byte[] rawtext = new byte[10000];
        int bytesread = 0, byteoffset = 0;
        int guess = 编码枚举.OTHER.ordinal();
    ...
    /**
     * Function : detectEncoding Aruguments: File Returns : One of the encodings from the Encoding enumeration (GB2312, HZ, BIG5, EUC_TW, ASCII, or OTHER) Description: This function looks at the file
     * and assigns it a probability score for each encoding type. The encoding type with the highest probability is returned.
     */
    public int 检测编码(File testfile) {
    ...
    /**
     * Function : detectEncoding Aruguments: byte array Returns : One of the encodings from the Encoding enumeration (GB2312, HZ, BIG5, EUC_TW, ASCII, or OTHER) Description: This function looks at the
     * byte array and assigns it a probability score for each encoding type. The encoding type with the highest probability is returned.
     */
    public int 检测编码(byte[] rawtext) {
        int[] scores;
        int index, maxscore = 0;
        int encoding_guess = 编码枚举.OTHER.ordinal();
        scores = new int[编码枚举.values().length];
        // Assign Scores
        scores[编码枚举.GB2312.ordinal()]       = gb2312_probability(rawtext);
        scores[编码枚举.GBK.ordinal()]          = gbk_probability(rawtext);
        scores[编码枚举.GB18030.ordinal()]      = gb18030_probability(rawtext);
        scores[编码枚举.HZ.ordinal()]           = hz_probability(rawtext);
        scores[编码枚举.BIG5.ordinal()]         = big5_probability(rawtext);
        scores[编码枚举.CNS11643.ordinal()]     = euc_tw_probability(rawtext);
        scores[编码枚举.ISO2022CN.ordinal()]    = iso_2022_cn_probability(rawtext);
        scores[编码枚举.UTF8.ordinal()]         = utf8_probability(rawtext);
        scores[编码枚举.UNICODE.ordinal()]      = utf16_probability(rawtext);
        scores[编码枚举.EUC_KR.ordinal()]       = euc_kr_probability(rawtext);
        scores[编码枚举.CP949.ordinal()]        = cp949_probability(rawtext);
        scores[编码枚举.JOHAB.ordinal()]        = 0;
        scores[编码枚举.ISO2022KR.ordinal()]    = iso_2022_kr_probability(rawtext);
        scores[编码枚举.ASCII.ordinal()]        = ascii_probability(rawtext);
        scores[编码枚举.SJIS.ordinal()]         = sjis_probability(rawtext);
        scores[编码枚举.EUC_JP.ordinal()]       = euc_jp_probability(rawtext);
        scores[编码枚举.ISO2022JP.ordinal()]    = iso_2022_jp_probability(rawtext);
        scores[编码枚举.UNICODET.ordinal()]     = 0;
        scores[编码枚举.UNICODES.ordinal()]     = 0;
        scores[编码枚举.ISO2022CN_GB.ordinal()] = 0;
        scores[编码枚举.ISO2022CN_CNS.ordinal()] = 0;
        scores[编码枚举.OTHER.ordinal()]        = 0;
        // Tabulate Scores
        for (index = 0; index < 编码枚举.values().length; index++) {
            if (debug) {
                System.err.println("Encoding " + 编码枚举.values()[index].友好名称 + " score " + scores[index]);
            }
        ...
        if (maxscore <= 50) {
            encoding_guess = 编码枚举.values().length;
        }
